...
func TestForAfterEach(t *testing.T) {
	t.Run("empty trie never calls the iterator", func(t *testing.T) {
		...
		trie.ForEachAfter(st, root, nil, func(key []byte, value store.Key) error {
			called = true
			return nil
		})
	...
	t.Run("when trie has two key:values", func(t *testing.T) {
		...
		t.Run("when I iterate after empty", func(t *testing.T) {
			...
			err = trie.ForEachAfter(st, root, nil, func(key []byte, value store.Key) error {
				keys = append(keys, key)
				values = append(values, value)
				return nil
			})
		...
		t.Run("when I iterate after the first", func(t *testing.T) {
			...
			err = trie.ForEachAfter(st, root, []byte{1, 2, 3}, func(key []byte, value store.Key) error {
				keys = append(keys, key)
				values = append(values, value)
				return nil
			})
		...
		t.Run("when I iterate after the second", func(t *testing.T) {
			...
			err = trie.ForEachAfter(st, root, []byte{1, 2, 4}, func(key []byte, value store.Key) error {
				keys = append(keys, key)
				values = append(values, value)
				return nil
			})
	...
	t.Run("when trie has three key:values", func(t *testing.T) {
		...
		t.Run("when I iterate after the second", func(t *testing.T) {
			...
			err = trie.ForEachAfter(st, root, []byte{1, 2, 3, 4}, func(key []byte, value store.Key) error {
				keys = append(keys, key)
				values = append(values, value)
				return nil
			})
